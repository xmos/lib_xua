...
constexpr int decimation_factor = (appconfSHF_NOMINAL_HZ == 32000) ? 3 : 6;
constexpr int stage_2_tap_count = (appconfSHF_NOMINAL_HZ == 32000) ? MIC_ARRAY_32K_STAGE_2_TAP_COUNT : STAGE2_TAP_COUNT;
constexpr const uint32_t* stage_1_filter() {
    return (appconfSHF_NOMINAL_HZ == 32000) ? &stage1_32k_coefs[0] : &stage1_coef[0];
}
constexpr const int32_t* stage_2_filter() {
    return (appconfSHF_NOMINAL_HZ == 32000) ? &stage2_32k_coefs[0] : &stage2_coef[0];
}
constexpr const right_shift_t* stage_2_shift() {
    return (appconfSHF_NOMINAL_HZ == 32000) ? &stage2_32k_shift : &stage2_shr;
}
