...
unsigned char xua_ep0_isConfigComplete = EP0_CONFIG_COMPLETE;
unsigned short xua_ep0_IdVendor = VENDOR_ID;             /* Vendor ID */
unsigned short xua_ep0_IdProductAudio1 = PID_AUDIO_1;    /* Product ID for Audio 1*/
unsigned short xua_ep0_IdProductAudio2 = PID_AUDIO_2;    /* Product ID for Audio 1*/
...
void XUA_Endpoint0_loop(XUD_Result_t result, USB_SetupPacket_t sp, chanend c_ep0_out, chanend c_ep0_in, chanend c_audioControl,
    chanend c_mix_ctl, chanend c_clk_ctl, chanend c_EANativeTransport_ctrl, CLIENT_INTERFACE(i_dfu, dfuInterface) VENDOR_REQUESTS_PARAMS_DEC_)
{
 // continue the loop only if the ep0 configuration is complete       
 if (xua_ep0_isConfigComplete==0) {
    return;
 }
...
#endif
#if (AUDIO_CLASS == 1)
            devDesc_Audio1.idVendor = xua_ep0_IdVendor;
            devDesc_Audio1.idProduct = xua_ep0_IdProductAudio1;
#else
            devDesc_Audio2.idVendor = xua_ep0_IdVendor;
            devDesc_Audio2.idProduct = xua_ep0_IdProductAudio2;
#endif // AUDIO_CLASS == 1
