...
#pragma select handler
static inline void CompleteSampleTransferNoUsbChans(chanend ?c_aud, const int readBuffNo, unsigned &command)
{
    if(XUA_USB_EN)
    {
        ...
        int seen_ct = testct(c_aud);

        if(seen_ct)
        {   /* Break with command set */
            command = (unsigned)inct(c_aud);
            return;
        }
        else
        {
            /* Consume single int back from decouple for case where we have no audio channels to decouple*/
            inuint(c_aud);
        }
    }
    /* Run user code */
...
static inline unsigned DoSampleTransfer(chanend ?c_aud, const int readBuffNo, const unsigned underflowWord)
{
    if(XUA_USB_EN)
    {
        ...
        {
            CompleteSampleTransferNoUsbChans(c_aud, readBuffNo, command); /* Check for command & UBM */
        }
        return command;
    }
    else
